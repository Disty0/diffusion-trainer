...
def get_flux2_embed_encoder(path: str, device: torch.device, dtype: torch.dtype, dynamo_backend: str) -> Tuple[Tuple[PreTrainedModel], Tuple[PreTrainedTokenizer]]:
    pipe = diffusers.Flux2KleinPipeline.from_pretrained(path, transformer=None, vae=None, torch_dtype=dtype)
...
def encode_flux2_embeds(embed_encoders: Tuple[Tuple[PreTrainedModel], Tuple[PreTrainedTokenizer]], device: torch.device, texts: List[str]) -> List[List[torch.FloatTensor]]:
