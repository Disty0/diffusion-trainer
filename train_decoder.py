...
def get_batches(batch_size: int, dataset_paths: List[Tuple[str, str, int]], dataset_index: str, image_ext: str, do_file_check: bool = True) -> None:
    ...
    for i in range(int(num_of_batches / 10000) + 1):
        os.makedirs(os.path.join(dataset_index, str(i)), exist_ok=True)

    for i, batch in enumerate(epoch_batch):
        with open(os.path.join(os.path.join(dataset_index, str(int(i / 10000))), str(i)+".json"), "w") as f:
            json.dump(batch, f)
...
def main() -> None:
    ...
    accelerator.print(f'Setting up dataset loader: dataset_index={config["dataset_index"]}')
    dataset = loader_utils.LatentDecoderDataset(config["dataset_index"], image_processor)
    accelerator.print(f'Using dataset loader: {dataset}')
    gc.collect()

    train_dataloader = DataLoader(dataset=dataset, batch_size=None, batch_sampler=None, shuffle=False, multiprocessing_context="spawn", pin_memory=config["dataloader_pin_memory"], num_workers=config["max_load_workers"], prefetch_factor=int(config["load_queue_lenght"]/(config["max_load_workers"] if config["max_load_workers"] != 0 else 1)))
